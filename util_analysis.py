...
#         print (eval_dict[str(qid)]["question"])
        #...  
#         context_len = len(eval_dict[str(qid)]["context"])
#         answer = eval_dict[str(qid)]["question"]
...
def eval_dicts(gold_dict, pred_dict, no_answer):
    avna = f1 = em = total = 0
    context_len_dict = np.zeros(400)
    
    for key, value in pred_dict.items():
        total += 1
        context_len_dict[min((len(gold_dict[key]['context']) - 1), 399)] +=1
...
def eval_class_dicts(gold_dict, pred_dict, no_answer):
    avna = np.zeros(8)
    f1 = np.zeros(8)
    em = np.zeros(8)
    total = np.zeros(8)
...
#         total += 1
        ...
        question_len = len(gold_dict[key]['question'])
        context_len = len(gold_dict[key]['context'])
        cur_class = None
        ...
        em[cur_class] += metric_max_over_ground_truths(compute_em, prediction, ground_truths)
        f1[cur_class] += metric_max_over_ground_truths(compute_f1, prediction, ground_truths)
        if no_answer:
            avna[cur_class] += compute_avna(prediction, ground_truths)
        total[cur_class] +=1
...
def eval_len_dicts(gold_dict, pred_dict, no_answer):
    avna = np.zeros(8)
    f1 = np.zeros(8)
    em = np.zeros(8)
    total = np.zeros(8)
